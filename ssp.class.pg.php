...
class SSP {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				if ( isset( $column['formatter'] ) ) {
					$row[ $column['dt'] ] =  pg_escape_string(utf8_encode(htmlentities($column['formatter']( $data[$i][ $c ], $data[$i] ))));
				}
				else {
					$row[ $column['dt'] ] =  pg_escape_string(utf8_encode(htmlentities($data[$i][ $c ])));
				}
			}
	...
	static function simple ( $request, $pg_details, $table, $primaryKey, $columns, $filtroAdd=NULL )
	{
		...
		if(is_bool($data)){
			...
			$dataResult = SSP::data_output( $columns, $data );
		}
